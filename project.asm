...
add_test:
   # Add a new medical test
    ...
    la $a0, id_buffer  # Load address of ic into $a0
    li $a1, 20  # Maximum number of characters to read
    li $v0, 8  # syscall 8 = read string
    syscall
   ...
   # Read name from user input
    la $a0, name_buffer  # Load address of name into $a0
    ...
    la $a0, date_buffer  # Load address of date into $a0
    li $a1, 100  # Maximum number of characters to read
    li $v0, 8  # syscall 8 = read string
    syscall
...
search_by_id:
    ...
    # Read id from user input
    la $a0, id_buffer  # Load address of id_buffer into $a0
    li $a1, 20  # Maximum number of characters to read
    li $v0, 8  # syscall 8 = read string
    syscall
...
search_unnormal:
    ...
    la $a0, name_buffer  # Load address of name_buffer into $a0
    li $a1, 100  # Maximum number of characters to read
    li $v0, 8  # syscall 8 = read string
    syscall
...
bpt:
    ...
    # Read result from user input
    la $a0, float1_buffer  # Load address of result into $a0
    li $a1, 9  # Maximum number of characters to read
    li $v0, 8  # syscall 8 = read string
    syscall
...
validate_name_loop:
    lb $t2, ($t1)  # Load byte from buffer
    beqz $t2, check_name_end  # End of string if byte is 0 (null terminator)
    addi $t0, $t0, 1   # Increment counter
    addi $t1, $t1, 1   # Move to next byte
...
check_name_end:
    ...
    name_loop:
        ...
        validate_char_loop:
            ...
            addi $t8, $t8, 1       # Move to next character in valid name
            addi $t9, $t9, 1       # Move to next character in input Test Name
...
end_find_records:
# After the loop, $t0 contains the address of the null terminator in records
# $t1 contains the offset where the new string should start
...
# Input: $a0 = address of the string buffer
...
# inputs : a1 : the address of the record to be copied
#		  a2 : the address of the destination memory location
# outputs: the record stored the destination memory location
#----------------------------------------------
copy_record:
...
# output : $a1 will point to the next record of the array
#--------------------------------------------------------
point_next_record:
	lbu $t0, ($a1)
	addi $a1, $a1, 1
...
# input	 : $a1: the pointer
# output : $a1 will point to the next field of the record
#--------------------------------------------------------
point_next_field:
    lbu $t0, ($a1)
    addi $a1, $a1, 1
...
# Input: $f12 = Hemoglobin (Hgb)
# Output: $v0 = (1 if abnormal, 0 if normal)
#---------------------------------------------------------------
check_Hgb:
...
# Input: $f12 = Blood Glucose Test (BGT)
# Output: $v0 = (1 if abnormal, 0 if normal)
#---------------------------------------------------------------
check_BGT:
...
# Input: $f12 = LDL Cholesterol Low-Density Lipoprotein (LDL)
# Output: $v0 = (1 if abnormal, 2 if normal)
#---------------------------------------------------------------
check_LDL:
...
# Input: $f7 = Systolic Blood Pressure, $f8 = Diastolic Blood Pressure
# Output: $v0 = (1 if abnormal, 0 if normal)
#---------------------------------------------------------------
check_BPT:
...
# and stop reading if found one of these charachters ", : \0 \r"
...
# input: name buffer
# output :$v0 = 1 if Hgb, 2 if BGT, 3 if LDL, 4 if BPT
#---------------------------------------------------------
check_test_type:
